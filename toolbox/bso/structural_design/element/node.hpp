...
namespace bso { namespace structural_design { namespace element {
	...
	public:
		...
		void updateNFT(const unsigned int& k, unsigned long& NFM); //will update the mNFT after tie method
		...
		const double& getX() const {return xCoordinate;}
		const double& getY() const {return yCoordinate;}
		const double& getZ() const {return zCoordinate;}
		
		bool mMasterTieNode = false;
		bool mSlaveTieNode = false;
		bool inTieCloud = false;
		
		const bool& isMasterTieNode() const {return mMasterTieNode;}
		const bool& isSlaveTieNode() const {return mSlaveTieNode;}
		const bool& TieCloudNode() const {return inTieCloud;}
		int CloudNodes = 0;
	};
