...
namespace bso { namespace structural_design {
	...
	void sd_model::mesh(const unsigned int& n, bool meshLoadPanels /* = true */)
	{
		...
		//tie method (create clouds)
		int numberOverlaps;
		for(auto& i : mGeometries)
		{
			if (i->isQuadrilateral())
			{
				component::quadrilateral* quad1 = dynamic_cast<component::quadrilateral*>(i);
				for(auto& j : mGeometries)
				{
					if (i == j){continue;}
					else if(j->OverlappingSurface==true&&i->OverlappingSurface==true)continue;//If both quadrilaterals are already tied together break
					else if (j->isQuadrilateral())
					{
						component::quadrilateral* quad2 = dynamic_cast<component::quadrilateral*>(j);
						for(auto& p : j->getElementPoints())
						{
							component::point* vertexPtr0 = dynamic_cast<component::point*>(p[0]);
							component::point* vertexPtr1 = dynamic_cast<component::point*>(p[1]);
							component::point* vertexPtr2 = dynamic_cast<component::point*>(p[2]);
							component::point* vertexPtr3 = dynamic_cast<component::point*>(p[3]);
							if(!quad1->isInsideOrOn(*vertexPtr0)||!quad1->isInsideOrOn(*vertexPtr1)||!quad1->isInsideOrOn(*vertexPtr2)||!quad1->isInsideOrOn(*vertexPtr3))continue;
							else
							{
								numberOverlaps++;
								//Initialize the larger elements
								double length1;
								double length2;
								for(auto& e : quad1->getElements())
								{
									element::flat_shell* ele1 = dynamic_cast<element::flat_shell*>(e);
									for(auto& l : ele1->getLines())
									{
										length1 = l.getLength();
										break;
									}
								...
								//Initial surface is the master surface when element size is larger
								bool mastersurface = false;
								if (length1>length2)
								{
									mastersurface=true;
								}

								//Element is found overlapping the first indicated quadrilateral
								std::vector<element::node*> tempNodes;
								//Adding nodes of first quadrilateral
								for(auto& n : i->getMeshedPoints())
								{
									auto NodeSearch = nodeMap.find(n);
									element::node* node = NodeSearch->second;
									if (node->inTieCloud==true){continue;}
									else
									{
										tempNodes.push_back(NodeSearch->second);
										if(mastersurface==true)
										{
											tempNodes.back()->mMasterTieNode = true;
											tempNodes.back()->mSlaveTieNode = false;
										}
										else 
										{
											tempNodes.back()->mMasterTieNode = false;
											tempNodes.back()->mSlaveTieNode = true;
										}
									}
								}
								//Adding nodes of second quadrilateral
								for(auto& n : j->getMeshedPoints())
								{
									auto NodeSearch = nodeMap.find(n);
									element::node* node = NodeSearch->second;
									if (node->inTieCloud==true){continue;}
								...
								i->OverlappingSurface=true;
								j->OverlappingSurface=true;

								//Determine the cloud radius for the tie method
								int radius;
								if(length1<length2){radius = sqrt(((length1/2)*(length1/2))+((length1/2)*(length1/2)))+1;}
								else{radius = sqrt(((length2/2))*((length2/2))+((length2/2))*((length2/2)))+1;}

								mFEA->tieMethod(tempNodes,radius);
								break;
							}
						}
					}
		...
		std::cout << "Number of overlaps: " << numberOverlaps << std::endl;

		//reduce stiffness of the "slave" elements
		mFEA->reduceStiffness();

		// generate the fea system
		mFEA->generateGSM();

		//Apply the tie method
		mFEA->rebuildmGSM();
		std::cout << "Tie method is done" << std::endl;
